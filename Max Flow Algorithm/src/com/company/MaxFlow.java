...
public class MaxFlow {
    ...
    private long MaxFlow; // Get the max flow

    // Get the maxflow
    public MaxFlow(FlowNetwork flowNetwork, Node source, Node sink) {
        ...
        while (hasAugmentingPath(flowNetwork, source, sink)) {
            ...
            for (Node n = sink; n != source; n = linkTo[n.getId()].getOtherNode(n)) { // Run for loop from sink to source till all the edges in the linkto list are got
            ...
            for (Node n = sink; n != source; n = linkTo[n.getId()].getOtherNode(n)) { // Run the for loop from sin to source again in order to add the residual flows to the original graph with bottleneck value, because there the same augmenting path applies to the original flownetwork
